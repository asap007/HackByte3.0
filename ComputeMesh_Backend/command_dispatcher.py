    ...
    async def receive_response(self, message: str):
        try:
            ...
            try:
                ...
                # If it's not JSON, treat it as a raw response
                data = {"result": message}
            ...
            if entry:
                ...
                if not future.done():
                    # Ensure we're setting a string/bytes if that's what's expected
                    if isinstance(result, (dict, list)):
                        future.set_result(json.dumps(result))
                    else:
                        future.set_result(str(result) if result is not None else "")
                    print(f"Received response for command_id '{command_id}' from user_id {user_id}")
            else:
